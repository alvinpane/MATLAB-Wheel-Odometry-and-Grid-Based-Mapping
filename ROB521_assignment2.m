...
for n=1:2
    ...
    for i=1:size(t_laser,1);
        ...
        if abs(omega_interp(i)) < 0.1
            %The second patch is to account for the fact that the origin of the 
            %laser scans is about 10 cm behind the origin of the robot.

            %Perform transformation to base and adjust by 10cm (0.1m)
            x = ((cos(angles+theta_interp(i))).*y_laser(i,:))' +  x_interp(i) - 0.1*cos(theta_interp(i));
            y = ((sin(angles+theta_interp(i))).*y_laser(i,:))' +  y_interp(i)- 0.1*sin(theta_interp(i));
